 ...
 * @param {boolean} [excludeNumericColumns=true] - When true, do not add filters for numeric columns.
 */
function setUpColumnFiltering(id, api, excludeNumericColumns = true) {
  const $filterHeaders = $(`${id} .filters th`);

  // Helper to determine whether a column is numeric.
  const isNumericColumn = (colIdx) => {
    if (!excludeNumericColumns) return false;
    const settings = api.settings()[0];
    const col =
      settings && settings.aoColumns ? settings.aoColumns[colIdx] : undefined;
    const sType = (col && (col.sType || col.type)) || "";
    if (typeof sType === "string") {
      const lowered = sType.toLowerCase();
      return (
        lowered === "num" || lowered === "numeric" || lowered.startsWith("num-")
      );
    }
    return false;
  };
  ...
  api
    ...
    .each(function (colIdx) {
      ...
      if (isNumericColumn(colIdx)) {
        // Skip creating a filter input for numeric columns when excluded.
        $cell.empty();
        return;
      }
      ...
      $cell.html(
        `<div class="ui input" style="width:100%"><input type="text" placeholder="${title}" /></div>`
      );
...
function renderList(data, type) {
  if (data && Array.isArray(data) && data.length > 0) {
    if (type === "filter") {
      ...
      return `<div class="ui list">${data
        .map((id) => `<div class="item">${id}</div>`)
        .join("")}</div>`;
    } else {
      return data[0];
    }
    ...
    if (Array.isArray(data) && data.length > 0) {
      if (type === "display") {
        return (
          header +
          data
            .map((item) => renderXoomReference(includeType)(item, "display"))
            .map(wrapUsed)
            .join(dividerUsed) +
          "</div>"
        );
        ...
        return data
          .map((item) => renderXoomReference(includeType)(item, type))
          .join("||");
      }
    }
    return null;
